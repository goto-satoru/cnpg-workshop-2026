...
### oc cnp による EPAS16 クラスタのステータス取得

```bash
oc -n edb cnp status epas16 -n edb

k -n edb cnp status epas16 -n edb

watch -n 5 oc -n edb cnp status epas16 -n edb
```
...
```bash
oc -n edb get secret epas16-app -o yaml | ./bin/decode-yaml.sh | grep password:
```
...
```bash
# スケジュールバックアップの確認
oc get scheduledbackup -n edb

# バックアップ履歴の確認
oc get backup -n edb

# バックアップの詳細確認
oc describe backup <backup-name> -n edb
```
...
```bash
# プライマリ Pod のログ
oc logs -n edb epas16-1 -f

# オペレーターのログ
oc logs -n postgresql-operator-system deployment/postgresql-operator-controller-manager -f
```
...
KIND クラスターの kubeconfig を取得し、oc で利用するには：
...
# Pod の詳細情報
oc describe pod <pod-name> -n edb

# イベント確認
oc get events -n edb --sort-by='.lastTimestamp'
```
...
```bash
# シークレットの確認
oc get secret edb-pull-secret -n edb

# シークレットの詳細確認
oc describe secret edb-pull-secret -n edb
...
# MinIO Pod の状態確認
oc get pods -n edb | grep minio

# MinIO のログ確認
oc logs -n edb <minio-pod-name>
```
...
# NodePort の確認
oc get svc epas16-rw -n edb -o jsonpath='{.spec.ports[0].nodePort}'

# ポートフォワーディングで直接接続を試す
oc port-forward -n edb svc/epas16-rw 5432:5432
```
